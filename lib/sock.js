...
const {
  ...
  const storeReadFromFile = (store) => store?.readFromFile(path.join('auth_files', 'baileys_store_multi.json'))

  const saveEveryTenSeconds = (store) => setInterval(() => {
    store?.writeToFile(path.join('auth_files', 'baileys_store_multi.json'))
  }, 10_000)
  ...
  const {
    state,
    saveCreds
  } = await useMultiFileAuthState(path.join('auth_files', 'baileys_auth_info'))
